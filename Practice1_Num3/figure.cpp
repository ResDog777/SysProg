...
void figure::show() {
	cout << "Четырёхугольник с вершинами: А(" << x1 << "," << y1 << ") " << "B(" << x2 << ", " << y2 << ") " << "C(" << x3 << ", " << y3 << ") " << "D(" << x4 << ", " << y4 << ") " << endl;
	cout << "Периметр: " << P << endl;
	cout << "Площадь: " << S << endl;
}
bool figure::is_prug() {
	if (abs(sqrt(pow(this->x1 - this->x3, 2) + sqrt(pow(this->y1 - this->y3, 2))) - sqrt(pow(this->x2 - this->x4, 2) + sqrt(pow(this->y2 - this->y4, 2)))) < 0.1)
	{
		return true;
	}
	else
...
bool figure::is_romb() {
	if (abs(sqrt(pow(this->x1 - this->x2, 2) + sqrt(pow(this->y1 - this->y2, 2))) - sqrt(pow(this->x2 - this->x3, 2) + sqrt(pow(this->y2 - this->y3, 2)))) < 0.1 &&
		abs(sqrt(pow(this->x2 - this->x3, 2) + sqrt(pow(this->y2 - this->y3, 2))) - sqrt(pow(this->x3 - this->x4, 2) + sqrt(pow(this->y3 - this->y4, 2)))) < 0.1 &&
		abs(sqrt(pow(this->x3 - this->x4, 2) + sqrt(pow(this->y3 - this->y4, 2))) - sqrt(pow(this->x4 - this->x1, 2) + sqrt(pow(this->y4 - this->y1, 2)))) < 0.1)
	{
		return true;
	}
	else
