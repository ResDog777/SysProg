...
int main()
{
	...
	while (true)
	{
		cout << "Введите количество дробей (не менее двух): " << endl;
		cin >> n;
		if (n < 2)
		{
			continue;
	...
	while (check1 == true)
	{
		cout << endl << "Выберите дроби для выбора операции -> от 1 до " << n << " Для выхода из программы введите ноль вместо одной из дробей-> 0." << endl;
		cin >> drob1 >> drob2;
		...
		{
			cout << "Ошибка. Такой дроби нет." << endl;
			continue;
		}
		...
		while (check2 == true)
		{
			...
			case 6:
				cout << "Присвоение дроби. Введите новые числители и знаменатели для двух дробей" << endl;
				cin >> f1 >> f2 >> f3 >> f4;
				tmp2.set(f1, f2);
				tmp3.set(f3, f4);
